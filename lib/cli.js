...
e.on("all users signed up", (str) => {
    cli.responders.allUsersSignedUp();
});

e.on("more details user", (str) => {
    cli.responders.moreDetailsUser(str);
});
...
cli.responders.help = () => {
    const commands = {
        ...
        'more details user --{email}': 'Lookup the details of a specific user by email address'
    };
    cli.horizontalLine();
    ...
    _data.list('users', (err, userIds) => {
        if (!err && userIds && userIds.length > 0) {
            ...
            userIds.forEach((userIds) => {
                _data.read('users', userIds, (err, userData) => {
                    // console.log(userData);
                    let dataLast24 = Date.now() - (1000 * 60 * 60 * 24);
                    if (!err && userData && dataLast24 < userData.time) {
                        let line =
                            'Name: ' + userData.name + ' Email: ' + userData.email + ' signed up time: ' + userData.time;
